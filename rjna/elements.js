...
function createTagWithState(tagName) {
  ...
  const setState = (newState, tag) => {
    //newState = {textContent:completed}
    //tag.iter(class:newState.class);
    //render()...
  ...
  const render = (tagObj =
    RJNA.createElement(tagName, attributes, eventHandlers, properties, ...children), oldNode) => {
    const node = RJNA.createNode(tagObj);
